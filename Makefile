...
desktop_heavy_x11_product=wkndr.mruby.x11
desktop_heavy_x11_target=$(build)/$(desktop_heavy_x11_product)
...
desktop_heavy_x11_objects += $(patsubst %,$(build)/desktop/%, $(patsubst %.c,%.o, $(sources)))
desktop_heavy_x11_objects += $(patsubst %,$(build)/desktop/%, $(patsubst %.cpp,%.o, $(cxx_sources)))
desktop_heavy_x11_objects += $(ode_static_lib)

desktop_heavy_x11_objects += $(patsubst %,$(build)/desktop/%, $(patsubst %.c,%.o, $(desktop_sources)))
desktop_heavy_x11_objects += $(desktop_heavy_mruby_static_lib)
desktop_heavy_x11_objects += $(desktop_heavy_msgpack_static_lib)
...
#.PHONY: $(desktop_heavy_target) $(desktop_heavy_x11_target)
...
$(desktop_heavy_x11_target): $(desktop_heavy_x11_objects)
	$(CC) $(CFLAGS) -o $@ $(desktop_heavy_x11_objects) $(LDFLAGS_X11)
...
clean:
	...
	cd raylib/src && make RAYLIB_RELEASE_PATH=../../$(build)/desktop-heavy PLATFORM=$(RAYLIB_PLATFORM_HEAVY) clean
	cd raylib/src && make RAYLIB_RELEASE_PATH=../../$(build)/desktop-heavy-x11 PLATFORM=$(RAYLIB_PLATFORM_HEAVY_X11) clean
	rm -R $(build)
	mkdir -p $(build)
	mkdir -p $(build)/src
	mkdir -p $(build)/src/desktop
	mkdir -p $(build)/desktop/src/desktop
	mkdir -p $(build)/desktop-heavy
